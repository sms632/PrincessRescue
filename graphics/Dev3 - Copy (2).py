...
class Cruiser(Ship):
    ...
    def __init__(self, x, y, char, owner):
    ...
    def update(self):
        if self.inputDelay > 0:
            ...
            if games.keyboard.is_pressed(games.K_SPACE):
                ...
                print(self.lowest)
                moveSum = abs(self.x - self.selector.x)/50 + abs(self.y - self.selector.y)/50
                if moveSum <= self.shipToMove.MOVE and self.overlapping_sprites[self.lowest - 1].nature == 1:
                    self.destroy()
...
class Game(object):
    ...
    def load(self):
        ...
        #add central island
        x = [350, 400, 350, 350, 400, 400, 450, 500, 350, 400, 450, 500]
        y = [350, 350, 400, 450, 450, 400, 450, 450, 500, 500, 500, 500]
        for i in range(12):
            island = Grid(x[i], y[i], nature = 2)
            games.screen.add(island)
            for sprite in island.overlapping_sprites:
                sprite.die()

        #add p1 ships
        cru1 = Cruiser(100, 250, self.player1, 1)
        self.p1Ships.append(cru1)
        cru2 = Cruiser(250, 100, self.player1, 1)
        self.p1Ships.append(cru2)
        car1 = Carrier(200, 150, self.player1, 1)
        self.p1Ships.append(car1)
        des1 = Destroyer(150, 200, self.player1, 1)
        self.p1Ships.append(des1)
        for ship in self.p1Ships:
            games.screen.add(ship)

        #add p2 ships
        cru1 = Cruiser(750, 600, self.player2, 2)
        self.p2Ships.append(cru1)
        cru2 = Cruiser(600, 750, self.player2, 2)
        self.p2Ships.append(cru2)
        car1 = Carrier(650, 700, self.player2, 2)
        self.p2Ships.append(car1)
        des1 = Destroyer(700, 650, self.player2, 2)
    ...
    def gameOver(self, winner):
        print("PLayer",winner,"has rescued their princess!")
